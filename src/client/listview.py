...
def cell_data_speed(column, cell, model, row, data):
    ...
    speed_str = ""
    if speed > 0:
        speed_str = str(speed)#deluge.common.fspeed(speed)
...
def cell_data_size(column, cell, model, row, data):
    ...
    size_str = str(size)#deluge.common.fsize(size)
...
def cell_data_time(column, cell, model, row, data):
    ...
    if time <= 0:
        ...
        time_str = str(time)#deluge.common.ftime(time)
...
def cell_data_date(column, cell, model, row, data):
    ...
    fdt = str(model.get_value(row, data))
    cell.set_property('text', fdt)
